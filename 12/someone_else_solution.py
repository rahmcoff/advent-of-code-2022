...
class Graph:
    ...
    def _get_neighbors(self, current: Tuple[int, int]) -> List[Tuple[int, int]]:
        ...
        neighbors = filter(
            lambda t: 0 <= t[0] < self._rows
                      and 0 <= t[1] < self._cols
    ...
    def print_distances(self):
        for row, line in enumerate(self._graph):
            print('  '.join([f'{height: >2} {self.distance_to.get((row, col), "-")!s: >3}'
...
def main():
    ...
    print(DATA.a_star_shortest_path(multiple_traversals=False))  # 517
    DATA.print_distances()
